...
public class EncodeParameter {
    public static String getLoginParameter(String phone_number, String passwd, String phone_mac, String imei, String model, String ssid) {
        String original_str = "phone_number=" + phone_number + "&passwd=" + Util.getMD5(passwd) + "&phone_mac="
                + phone_mac + "&imei=" + imei + "&model=" + model + "&ssid=" + Util.getMD5(ssid) + getPublicParameter();
    ...
    private static String getPublicParameter() {
        return "&pl=" + Constants.PLATFORM + "&ver=" + Util.getVersionCode() + "&channel=" + ApplicationInfo.channel + "&t=" + System.currentTimeMillis();
    }
}
